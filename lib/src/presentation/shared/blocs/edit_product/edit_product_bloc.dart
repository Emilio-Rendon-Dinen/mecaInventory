...
class EditProductBloc
    extends Bloc<EditProductRequestedEvent, EditProductState> {
  EditProductBloc() : super(EditProductInitial()) {
    on<EditProductRequestedEvent>(_onEditProductLoading);
  }

  Future<void> _onEditProductLoading(
      EditProductRequestedEvent event, Emitter<EditProductState> emit) async {
    emit(const EditProductLoading());
    try {
      final ProductUIModel productUIModel =
          ProductUIModel.fromDomain(await event.useCase.editProduct(
