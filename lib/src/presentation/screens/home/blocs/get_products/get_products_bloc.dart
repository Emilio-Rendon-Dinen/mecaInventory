...
class GetProductsBloc extends Bloc<GetProductsEvent, GetProductsState> {
  ...
  Future<void> _onGetProductsLoading(
      GetProductsDataEvent event, Emitter<GetProductsState> emit) async {
  ...
  Future<void> _onUpdateProductInList(
      UpdateProductInListEvent event, Emitter<GetProductsState> emit) async {
    if (state case GetProductsSuccess(:final products)) {
      final updatedList = products.map((product) {
        return product.id == event.updatedProduct.id
